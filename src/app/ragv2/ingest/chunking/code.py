...
class CodeChunker:
    """Splits code around language-specific structural boundaries."""
    ...
    def chunk(self, document: ParsedDocument) -> tuple[Chunk, ...]:
        ...
        language = str(document.metadata.get("language", "unknown"))
        splitter = self._create_splitter(language)
        chunks = []

        for section in document.sections:
            split_documents = splitter.create_documents([section.text])
    ...
    def _create_splitter(
        ...
        common_settings = {
            "chunk_size": self.settings.child_chunk_tokens,
            "chunk_overlap": self.settings.child_chunk_overlap_tokens,
            "length_function": self.token_counter.count,
            "keep_separator": "end",
            "add_start_index": True,
        }

        langchain_language = LANGCHAIN_LANGUAGES.get(language)
